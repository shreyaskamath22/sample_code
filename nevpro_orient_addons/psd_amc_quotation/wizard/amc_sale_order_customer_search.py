...
class psd_amc_sale_customer_search_wizard(osv.osv_memory):
    ...
    _columns = {
        ...
        'psd_amc_sale_customer_search_line': fields.one2many('psd.amc.sale.customer.request.line', 'product_req_customer_search_id', 'Customers'),
    }    

    def default_get(self,cr,uid,fields,context=None):
        customer_line_ids =[]
        if context is None: context = {}
        res = super(psd_amc_sale_customer_search_wizard, self).default_get(cr, uid, fields, context=context)
        active_ids = context.get('active_ids')
        if active_ids:
            active_id = active_ids[0]
            product_req_obj = self.pool.get('amc.sale.order')
            partner_obj = self.pool.get('res.partner')
            psd_customer_request_line = self.pool.get('psd.amc.sale.customer.request.line')
        ...
        if 'name' in fields:
            picking=self.pool.get('amc.sale.order').browse(cr,uid,picking_id,context=context)
            res.update(name=picking.customer_name)
        if 'psd_amc_sale_customer_search_line' in fields:
            picking = self.pool.get('psd.amc.sale.customer.request.line').browse(cr, uid, picking_id, context=context)
    ...
    def product_request_search_customer(self,cr,uid,ids,context=None):
        ...
        loc_line_ids = []
        locations_line_ids = self.browse(cr,uid,ids[0]).psd_amc_sale_customer_search_line
        if locations_line_ids:
            for locations_line_id in locations_line_ids:
                loc_line_ids.append(locations_line_id.id)
        self.pool.get('psd.amc.sale.customer.request.line').unlink(cr, uid, loc_line_ids, context=context)
        partner_obj = self.pool.get('res.partner')
        psd_customer_request_line = self.pool.get('psd.amc.sale.customer.request.line')
        ...
        for customer_id in customer_ids:
            ...
            self.pool.get('psd.amc.sale.customer.request.line').create(cr,uid,{'name': partner.name,
...
psd_amc_sale_customer_search_wizard()



class psd_amc_sale_customer_request_line(osv.osv_memory):
    _name='psd.amc.sale.customer.request.line'  

    _columns = {
        'product_req_customer_search_id': fields.many2one('psd.amc.sale.customer.search.wizard', 'Customer Search'),
...
psd_amc_sale_customer_request_line()   
