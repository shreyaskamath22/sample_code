...
class stock_partial_picking_line(osv.TransientModel):
    ...
    _columns = {
        'product_billed_qty' : fields.float('Billed Quantity'),
    }
...
class stock_partial_picking(osv.osv_memory):
    ...
    _columns = {
        'partial_picking': fields.boolean('Partial Delivery'),
        'partial_date': fields.date('Partial Shipment Date'),
    }
    ...
    def _partial_move_for(self, cr, uid, move, context=None):
        partial_move = {
            ...
            'product_billed_qty': move.product_billed_qty,
    ...
    def do_partial(self, cr, uid, ids, context=None):
        ...
        partial_date = partial.partial_date
        cr.execute('select count(*) from stock_move where picking_id =%s',(partial_picking_id,))
        stock_move_values = map(lambda x: x[0], cr.fetchall())
        stock_move_values_count = stock_move_values[0]
        wizard_move_values_count = len(partial.move_ids)
        for wizard_line in partial.move_ids:
            line_uom = wizard_line.product_uom
            move_id = wizard_line.move_id.id
            product_billed_qty = wizard_line.product_billed_qty
            product_move_quantity = wizard_line.move_id.product_qty
            product_move_billed_quantity = wizard_line.move_id.product_billed_qty

            #Billed Quantity must be less or equal to the move billed quantity of the product
            if product_move_billed_quantity < wizard_line.product_billed_qty:
                raise osv.except_osv(_('Warning!'), _('The Billed Quantity will not be greater than the original ordered Billed quantity.'))

            #Shipped Quantity must be less or equal to the move shipped quantity of the product
            if product_move_quantity < wizard_line.quantity:
                raise osv.except_osv(_('Warning!'), _('The Shipped Quantity will not be greater than the original ordered Shipped quantity.'))

            #Quantiny must be Positive
            if wizard_line.quantity <= 0 or wizard_line.product_billed_qty <= 0:
                raise osv.except_osv(_('Warning!'), _('Please provide proper Quantity.'))
            
            if partial_date != False:
                pass
            elif product_move_billed_quantity != wizard_line.product_billed_qty or product_move_quantity != wizard_line.quantity or stock_move_values_count > wizard_move_values_count:
                raise osv.except_osv(_('Warning!'), _("Since it is a case of Partial Delivery, please tick the checkbox for 'Partial Delivery' and enter Partial Shipment Date of the backorder"))
