...
class hr_holidays(osv.osv):
    ...
    def carry_forwarding_leaves(self, cr, uid, ids=None, context=None):
        ...
        search_emp_record = self.pool.get('hr.employee').search(cr, uid, [('id', '>', 0),('active','=',True)], context=context)

        
        print search_emp_record, "yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy"

        for record in self.pool.get('hr.employee').browse(cr, uid, search_emp_record):
            
            joining_date = record.join_date
            form_id = record.id
            print joining_date, "kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"
            
            joining_date_year = int(joining_date[:4])
            joining_date_month = int(joining_date[5:7])
            joining_date_date = int(joining_date[8:10]) 

            date_after_1year_of_joining_date = date(joining_date_year,joining_date_month,joining_date_date) + relativedelta(years=+1)
            
            date_after_1year_of_joining_date_str = str(date_after_1year_of_joining_date)
            
            print date_after_1year_of_joining_date, type(date_after_1year_of_joining_date), "llllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllll"
            
            print today_date, type(today_date), "--------------------------------------------------------------------------------------------"
            
            search_privilege_leave = self.pool.get('hr.holidays.status').search(cr,uid,[('name','=','Privilege Leave (PL)')], context=context)
            
            if today_date < date_after_1year_of_joining_date_str:
                print "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
                sick_leave_carry_forward_write_id = self.pool.get('hr.employee').write(cr, uid, form_id, {'sick_leaves': 0})
                
                search_privilege_leave_in_allocation_requests = self.pool.get('hr.holidays').search(cr,uid,[('holiday_status_id','=',search_privilege_leave[0]),('type','=','add'),('employee_id','=',form_id)], context=context)
                
                for l in self.pool.get('hr.holidays').browse(cr, uid, search_privilege_leave_in_allocation_requests):
                    leave_to_be_refused_id = l.id
                    
                    self.pool.get('hr.holidays').holidays_refuse(cr, uid, [leave_to_be_refused_id], context=context)    
                
                
                
                print search_privilege_leave_in_allocation_requests, form_id, "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Privilege Leaves in Allocation Requests^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
                
            casual_leave_carry_forward_write_id = self.pool.get('hr.employee').write(cr, uid, form_id, {'casual_leaves': 0})
    ...
    def allocate_sick_leaves_annually(self,cr,uid,ids=None,context=None):
        ...
        emp_search = self.pool.get('hr.employee').search(cr, uid, [('id', '>', 0),('active','=',True)], context=context)
        ...
        for m in self.pool.get('hr.employee').browse(cr, uid, emp_search):
            ...
            print create_id, "lllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllll"   
            self.pool.get('hr.holidays').holidays_validate(cr, uid, [create_id], context=context)
                #self.pool.get('hr.holidays').holidays_validate(cr, uid, [create_id], context=context)
            ...
            print create_casual_id, "nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn"       
               
               
            self.pool.get('hr.holidays').holidays_validate(cr, uid, [create_casual_id], context=context)
            
            create_privilege_id = self.pool.get('hr.holidays').create(cr, uid, {'name': 'Allocation of 22 PLs annually', 'holiday_status_id': search_privilege_leave[0], 'employee_id': m.id, 'number_of_days_temp': 22, 'type': 'add'}, context=context)

            print create_casual_id, "nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn"       
               
               
            self.pool.get('hr.holidays').holidays_validate(cr, uid, [create_privilege_id], context=context)
                #self.pool.get('hr.holidays').holidays_validate(cr, uid, [create_casual_id], context=context)
